...
var Player = preload("res://src/scripts/world/player.gd").new()
...
func player_move_update(hud):
	...
	if true: #Maanager.get_entities_with("Interfaces"):
		...
		var normal = Player.get_looking_at_normal(player, OS.get_window_size() / 2)
		var block_location = Player.get_looking_at(player, OS.get_window_size() / 2)# - normal
		...
		var location = ChunkTools.get_chunk(block_location)
...
func chunk_info_update(hud, player_pos):
	...
	var chunk_loc = ChunkTools.get_chunk(player_pos)
