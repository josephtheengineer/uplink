...
static func can_be_seen(position: Vector3, block_data: Dictionary):
...
static func create_cube(position: Vector3, id: int, mesh, materials: Dictionary, block_data: Dictionary):
	var vertex_data = []
...
static func create_vertical_plane(st, position, direction):
	var vertex_data = []
	if direction == "west":
...
static func create_horizontal_plane(st, position, direction):
	var vertex_data = []
	if direction == "up":
