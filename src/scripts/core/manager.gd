...
static func create(entity: Dictionary): ###############################################
	Core.emit_signal("msg", "Creating new Entity, " + entity.type, Core.TRACE, meta)
	if entity.type == "interface":
		...
		if entity.name_id == "tty":
			...
			var tty := preload("res://src/scenes/terminal/tty.tscn")
		...
		if entity.name_id == "hud":
			...
			var hud := preload("res://src/scenes/hud/hud.tscn")
		...
		if entity.name_id == "player":
			...
			var player := preload("res://src/scenes/world/player.tscn")
...
#static func edit(id, components): ####################################################
#	var entity = Dictionary()
#
#	entity.id = id
#	entity.components = components
...
#	return entity.id
#
#static func get_component(id, path): #################################################
...
#static func add_node(id, component, node): ###########################################
...
#	else:
#		Core.emit_signal("msg", "Parent node doesn't exist yet!", Core.WARN, meta)
#		return false
#
...
#	return path
#
#static func inherit_child_rect(id, component): # component string ####################
...
#static func setup_scripts(aux: Dictionary, scripts: Array):
#	for script in scripts:
#		var script_file: String = script.replace(".", "/")
#		var script_base: Array = script.split(".")
#		for i in range(script_base.size() - 1):
#			aux[script_base[i]] = Dictionary()
#		aux[script_base[0]][script_base[1]] = load("res://src/scripts/" + script_file + ".gd")
#	return aux
